      ...
      <div class="row">
        ...
        <div class="col-md-4" v-if="showOptions">
          <div class="lbpradio">
              ...
              <div class="lbpradio-danger">
                  <input @click="showCorrelation" type="radio" name="radio" id="radio3"  v-model="checks[2]" value="2" />
                  <label for="radio3">Correlación</label>
              </div>
              <div class="lbpradio-danger">
                  <input @click="showDataCorrelation" type="radio" name="radio" id="radio4"  v-model="checks[3]" value="3" />
        ...
        <div class="col-md-4" v-if="showCorrelationType">
          <div class="row">
            <div class="col-12">
              <div class="form-group">
                <label class="bmd-label-floating">Por Correlación</label>
                <select @change="selectByCorrelation" v-model="CorrelationPicked"  id="CorrelationPicker" class=" form-control">
                    <option v-for="l in Correlation" :key="l.id" :value="l.id">{{ l.name }}</option>
                </select>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods:{
    ...
    showStrutureInformation(){
      ...
      this.showCorrelationType=false
      this.showUserType=false
    },
    showCorrelation(){
      this.loadCatalogCorrelation()
      this.showCorrelationType=true
      this.showProductType=false
      this.showUserType=false
    },
    showUsers(){
    ...
    selectByProduct(){
      let me = this
      axios.get('/grafica/correlacion/productos/', {
        params: {
          project_id: me.projectPickedId,
      ...
      .then(response => {
        ...
        this.tipo = "Correlación por "+product
        this.dataToShowInGraph =  response.data
      });
    },
    selectByLevel(){
      let me = this
      axios.get('/grafica/correlacion/niveles/', {
        params: {
          project_id: me.projectPickedId,
      ...
      .then(response => {
        this.tipo = "Correlación por nivel: "+me.levelPicked
        this.dataToShowInGraph =  response.data
      });
    },
    selectByCorrelation(){
      let me = this
      axios.get('/grafica/correlacion', {
        params: {
          project_id: me.projectPickedId,
          correlation: me.CorrelationPicked
        }
      })
      .then(response => {
        this.tipo = "Tareas por Correlación: "+me.CorrelationPicked
        this.dataToShowInGraph =  response.data
      });
    },
    getUserData(user){
      let me = this
      axios.get('/grafica/correlacion/usuario/', {
        params: {
          user_id: user.id
        }
      })
    ...
    getTableData(){
      let me = this
      axios.get('/grafica/correlacion/datos/', {
        params: {
          project_id: me.projectPickedId
        }
      })
      .then(response => {
        if(Object.keys(response.data.content).length > 0){
          this.showNodata=false
          this.tableName = "Datos de usuarios, tareas y Correlación"
