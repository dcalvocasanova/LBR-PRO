      ...
      <div class="row">
        ...
        <div class="col-md-4" v-if="showOptions">
          <div class="lbpradio">
              ...
              <div class="lbpradio-danger">
                  <input @click="showAddedValue" type="radio" name="radio" id="radio3"  v-model="checks[2]" value="2" />
                  <label for="radio3">Valor Agregado</label>
              </div>
              <div class="lbpradio-danger">
                  <input @click="showDataAddedValue" type="radio" name="radio" id="radio4"  v-model="checks[3]" value="3" />
        ...
        <div class="col-md-4" v-if="showAddedValueType">
          <div class="row">
            <div class="col-12">
              <div class="form-group">
                <label class="bmd-label-floating">Por Valor Agregado</label>
                <select @change="selectByAddedValue" v-model="AddedValuePicked"  id="AddedValuePicker" class=" form-control">
                  <option id="1" value="1"> Valor Agregado Real</option>
                  <option id="2" value="2"> Valor Agregado para empresa</option>
                  <option id="3" value="3"> Sin Valor Agregado</option>
                </select>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods:{
    ...
    showStrutureInformation(){
      ...
      this.showAddedValueType=false
      this.showUserType=false
    },
    showAddedValue(){
      this.loadCatalogAddedValue()
      this.showAddedValueType=true
      this.showProductType=false
      this.showUserType=false
    },
    showUsers(){
    ...
    loadCatalogAddedValue(){
      axios.get('/catalogo?id=ADDED-VALUED-T')
      .then(response => {
        this.AddedValue = response.data;
      });
    },
    selectByProduct(){
      let me = this
      axios.get('/grafica/valor-agregado/productos/', {
        params: {
          project_id: me.projectPickedId,
      ...
      .then(response => {
        ...
        this.tipo = "Valor Agregados por "+product
        this.dataToShowInGraph =  response.data
      });
    },
    selectByLevel(){
      let me = this
      axios.get('/grafica/valor-agregado/niveles/', {
        params: {
          project_id: me.projectPickedId,
      ...
      .then(response => {
        this.tipo = "Valor Agregados por nivel: "+me.levelPicked
        this.dataToShowInGraph =  response.data
      });
    },
    selectByAddedValue(){
      let me = this
      axios.get('/grafica/valor-agregado', {
        params: {
          project_id: me.projectPickedId,
          addedValue: me.AddedValuePicked
        }
      })
      .then(response => {
        this.tipo = "Tareas por Valor Agregado: "+me.AddedValuePicked
        this.dataToShowInGraph =  response.data
      });
    },
    getUserData(user){
      let me = this
      axios.get('/grafica/valor-agregado/usuario/', {
        params: {
          user_id: user.id
        }
      })
    ...
    getTableData(){
      let me = this
      axios.get('/grafica/valor-agregado/datos/', {
        params: {
          project_id: me.projectPickedId
        }
      })
      .then(response => {
        if(Object.keys(response.data.content).length > 0){
          this.showNodata=false
          this.tableName = "Datos de usuarios, tareas y Valor Agregados"
